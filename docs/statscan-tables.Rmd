...
In this project, the xls files are cleaned and merged into one csv sheet for all years using open source RStudio. The cleaned data are in tidied folder and the codes used to wrangle the data are within R script folder respectively.
...
Currently, the fully anonymized data includes area-based income estimates for individuals ranging from years 2000 up to 2015. Each table in a year is an aggregation of sub-tables as listed in Table 1. The anonymized income estimates for families range from years 2004 up to 2016. Each table in a year is an aggregation of all sub-tables as listed in Table 2. 

The original data came in xls format with spaces in names. RStudio was used to wrangle the data into machine readable csv's. 
